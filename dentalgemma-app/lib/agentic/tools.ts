...
export async function assessCase(input: AssessCaseInput) {
  try {
    ...
    const caseData: ClinicalCase = {
      ...
      chiefComplaint: {
        description: `${input.chiefComplaint}. Duration: ${input.duration}. Pain level: ${input.painLevel}/10.`,
      },
      history: input.duration ? `Symptoms present for ${input.duration}.` : 'No history provided',
      clinicalFindings: {
        description: input.clinicalFindings,
      },
      radiographicFindings: {
        description: input.radiographicFindings || 'None',
      },
      medicalHistory: {
        history: 'Not specified',
        systemicConditions: input.medicalHistory || 'None reported',
        medications: 'None',
        habits: 'None',
      },
      createdAt: new Date(),
